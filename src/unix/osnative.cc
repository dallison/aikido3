...
// set the operating system name
    ...
    mib[1] = KERN_VERSION ;
    ln = sizeof (version) ;
    sysctl (mib, 2, version, &ln, NULL, 0) ;
    
    setSysVar ("osinfo",  version, staticLink) ;
    ...
    mib[1] = HW_MEMSIZE ;
    ln = sizeof (val64) ;
    sysctl (mib, 2, &val64, &ln, NULL, 0) ;
    setSysVar ("numpages", (INTEGER)val64/pagesize, staticLink) ;
...
AIKIDO_NATIVE (execvp) {
    if (b->properties & LOCKDOWN) {
        throw newException (vm, stack, "Security violation") ;
    }
    if (paras[1].type != T_STRING) {
        throw newParameterException (vm, stack, "execvp", "Bad command parameter type") ;
        return 0 ;
    }
    if (paras[2].type != T_VECTOR) {
        throw newParameterException (vm, stack, "execvp", "Bad args parameter type") ;
        return 0 ;
    }
    Value::vector *argv = paras[2].vec ;
    const char **args = new const char *[argv->size() + 1] ;
    ...
    args[argv->size()] = NULL ;
    execvp (paras[1].str->c_str(), (char*const*)args) ;
    return 0 ;
}

AIKIDO_NATIVE (execlp) {
    if (b->properties & LOCKDOWN) {
        throw newException (vm, stack, "Security violation") ;
    }
    if (paras[1].type != T_STRING) {
        throw newParameterException (vm, stack, "execlp", "Bad command parameter type") ;
        return 0 ;
    }
    const char **args = new const char *[paras.size() - 2 + 1] ;
    for (int i = 2 ; i < paras.size() ; i++) {
        if (paras[i].type != T_STRING) {
            throw newParameterException (vm, stack, "execlp", "Bad args parameter type") ;
        }
        args[i-2] = paras[i].str->c_str() ;
    }
    args[paras.size()-2] = NULL ;
...
AIKIDO_NATIVE(pipeclose) {
    ...
    int pid = (int)((unsigned long)s->getThreadID() & 0xffffffffl) ;
    s->close() ;
    int statloc ;
    waitpid (pid, &statloc, 0) ;
    if (WIFEXITED(statloc)) {
...
AIKIDO_NATIVE (kill) {
    ...
    if (paras[1].type == T_STREAM) {
        int pid = (int)((unsigned long)paras[1].stream->getThreadID() & 0xffffffffl) ;
...
AIKIDO_NATIVE (opentty) {
    if (paras[1].type != T_STRING) {
        throw newParameterException (vm, stack, "opentty", "Illegal command value") ;
    }
    if (paras[2].type != T_VECTOR) {
        throw newParameterException (vm, stack, "opentty", "Illegal args value") ;
    }

    std::string command = paras[1].str->str ;
    ...
    int master = 0 ;
    char slave[256] ;
    pid_t pid = forkpty (&master, slave, NULL, NULL) ;
    if (pid == 0) {
        // now we exec the  program
        const char **argv = new const char * [args->size() + 2] ;
        argv[0] = command.c_str() ;
        for (unsigned int i = 0 ; i < args->size() ; i++) {
            argv[i+1] = (*args)[i].str->c_str() ;
        }
        argv[args->size()+1] = NULL ;
        execvp (command.c_str(), (char*const*)argv) ;
        exit (1) ;
    } else if (pid < 0) {
        throw newException (vm, stack, "Failed to open pseudo-tty (fork)") ;
    } else {
        // return vector of [master, pid]
        Value::vector *r = new Value::vector() ;
        r->push_back (Value (master)) ;
        r->push_back (Value (pid)) ;
        return r ;
    }

#elif defined(_OS_LINUX) || defined(_OS_SOLARIS)
    int master = getpt() ;

    if (master < 0) {
        throw newException (vm, stack, "Failed to open pseudo-tty (getpt)") ;
    }
    // printf ("master pty = %d\n", master) ;

    // call the functions the manual says to call for some reason
    int e = grantpt (master) ;
    if (e < 0) {
        throw newException (vm, stack, "Failed to open pseudo-tty (grantpt)") ;
    }
    e = unlockpt (master) ;
    if (e < 0) {
        throw newException (vm, stack, "Failed to open pseudo-tty (unlockpt)") ;
    }

    // get the name of the associated slave terminal
    char *slavename = ptsname (master) ;
    if (slavename == NULL) {
        throw newException (vm, stack, "Failed to open pseudo-tty (ptsname)") ;
    }
    ...
    if (pid == 0) {
        ...
        int slave = open (slavename, O_RDWR, 0777) ;
        if (slave < 0) {
            // failed to open slave terminal, exit with error
            exit (2) ;
        }
        // printf ("slave: %d\n", slave) ;

        // "A careful  programmer  will  not  use  dup2 without closing newfd first" from dup2(2)
        close (0) ;
        close (1) ;
        close (2) ;

        // redirect input, output and error to the slave tty
        dup2 (slave, 0) ;           // standard input to pseudotty
        dup2 (slave, 1) ;           // standard output to pseudotty
        dup2 (slave, 2) ;           // and error
        close (slave) ;
        // now we exec the  program
        const char **argv = new const char * [args->size() + 2] ;
        argv[0] = command.c_str() ;
...
AIKIDO_NATIVE (closetty) {
    ...
    if (paras[1].type != T_VECTOR) {
        throw newParameterException (vm, stack, "closetty", "Illegal descriptor value") ;
    }

    int master = (*paras[1].vec)[0].integer ;
    int pid = (*paras[1].vec)[1].integer ;

    close (master) ;

    kill (pid, SIGTERM) ;       // kill the child (if it hasn't already gone away)
...
AIKIDO_NATIVE(getProcessMemorySize) {
    ...
    int bytes = t_info.resident_size;
...
AIKIDO_NATIVE(threadSleep) {
    ...
    INTEGER us = getInt (paras[0]) ;

    const int qmax = 500000 ;
    while (us > 0 && !interrupted) {
        int quantum = us > qmax ? qmax : us ;     // one quantum is 10000us (10ms)
#if defined(_OS_MACOSX) 
        usleep (quantum) ;         // 10ms
#else
        struct timespec ts;
